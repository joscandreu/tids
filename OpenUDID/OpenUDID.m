...
@interface NetIamyellowTidsModule_OpenUDID (Private)
...
@implementation NetIamyellowTidsModule_OpenUDID
...
# error                                                         ^ read comments above, fix accordingly, and remove this #error line
  ...
  */
    ...
    if (_openUDID==nil) {
        ...
        CFRelease(uuid);
        CFRelease(cfstring); // leak https://github.com/dxt/OpenUDID/commit/4bd756e79dd1d88a7a907515654b6ccb29213f48
...
//
+ (NSString*) value {
    return [NetIamyellowTidsModule_OpenUDID valueWithError:nil];
}
...
+ (NSString*) valueWithError:(NSError **)error {
    ...
    for (int n=0; n<kOpenUDIDRedundancySlots; n++) {
        ...
        if (slotPB==nil) {
            ...
            NSDictionary* dict = [NetIamyellowTidsModule_OpenUDID _getDictFromPasteboard:slotPB];
    ...
    if (openUDID==nil) {        
        if (mostReliableOpenUDID==nil) {
            ...
            //
            openUDID = [NetIamyellowTidsModule_OpenUDID _generateFreshOpenUDID];
        } else {
    ...
    if (availableSlotPBid!=nil && (myRedundancySlotPBid==nil || [availableSlotPBid isEqualToString:myRedundancySlotPBid])) {
        ...
        if (openUDID && localDict)
            [NetIamyellowTidsModule_OpenUDID _setDict:localDict forPasteboard:slotPB];
    }
...
+ (void) setOptOut:(BOOL)optOutValue {
    ...
    // init call
    [NetIamyellowTidsModule_OpenUDID value];
